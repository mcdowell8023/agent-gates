...
## Cross-review capability detection (v1.6.0)

Starting with v1.6.0, both `install.sh` and `doctor.sh` detect available heterogeneous review tools and persist the results.

### review-capability.json

**Location**: `~/.agent-gates/review-capability.json` (runtime-generated, not in the repo).

**Generated by**: `install.sh` (`detect_review_capability` function) on first install/upgrade, and `doctor.sh` (`check_cross_review_capability` function) on every health check run.

**Read by**: `agent-review-protocol` SKILL.md §8 (Cross-Check Platform Routing) — the skill reads this file at review time to determine which review route to use, avoiding runtime tool detection.
...
```json
{
  ...
  "env": "local",
  "decision_tree": [
    { "route": "opencode", "available": true, "path": "/usr/local/bin/opencode" },
    { "route": "codex", "available": true, "version": "0.1.2504301", "path": "/usr/local/bin/codex" },
    { "route": "omc-codex-plugin", "available": false },
    { "route": "paseo", "available": true },
...
| Level | Meaning | Tools detected |
|-------|---------|----------------|
| L3 | Full heterogeneous | opencode + codex both available |
| L2 | Single heterogeneous | opencode available |
| L1 | Single heterogeneous | codex or OMC codex plugin available |
| L0 | Same-model only | No heterogeneous tools; falls back to agent-tool (same model review) |

**Environment types**: `local` (default), `ci` (CI detected via `$CI`), `container` (Docker detected via `/.dockerenv`), `windows` (Cygwin/MINGW/MSYS), `wsl` (Windows Subsystem for Linux). The environment value is recorded for review routing to adapt to constrained environments.

**Paseo**: Detected and recorded in `decision_tree` but does not affect the capability level (it is an orchestration layer, not a review tool).

### doctor.sh: check_cross_review_capability

Added in v1.6.0. Runs as the last check in the doctor sequence. Probes all five review routes, computes the capability level, writes `review-capability.json`, and reports:

- **PASS** for L1--L3 (at least one heterogeneous tool detected)
- **WARN** for L0 (no heterogeneous tools; prints upgrade suggestions for opencode/codex)
- **note** when Paseo is detected or when a non-local environment is identified

Additionally checks `.agent/reviews/` for recent review files (last 7 days) that contain `REVIEW_LEVEL: L0` and warns if found (single-model review does not satisfy the heterogeneous requirement).

### install.sh: detect_review_capability

Added in v1.6.0. Runs at the end of the install flow (after `install_external_deps`). Same detection logic as `check_cross_review_capability` but with install-time messaging: at L0, prints detailed guidance on installing opencode CLI or codex CLI to enable heterogeneous reviews.

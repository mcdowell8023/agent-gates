...
agent-gates 就是这一层：把工程纪律编码成 **五个 skill + 两个 hook + 四个 CLI 工具 + 一个项目模板**，配套自动安装器（install.sh）、自检工具（doctor.sh）、卸载器（uninstall.sh）。
...
flowchart TB
    ...
    subgraph skills["5 个 skill（agent 运行时自动加载）"]
        S1[init-project-gates<br/>项目初始化器]
        S2[agent-workflow-rules<br/>TDD / 验证 / 防循环]
        S3[agent-review-protocol<br/>三 agent 审查 + 交叉检查]
        S4[init-deep-fallback<br/>层级 AGENTS.md 兜底]
        S5[memory<br/>内置 Memory skill]
    end

    subgraph hooks["2 个 hook（事件触发）"]
        H1[memory-reminder.mjs<br/>PostToolUse: TaskUpdate completed<br/>+ Parallelism Reminder]
    ...
    subgraph runtime["每仓库运行时（v1.9.0+ shim 模型）"]
        R1[.agent/PROGRESS.md<br/>.agent/reviews/<br/>.agent/memory/]
        R2[.githooks/agent-quality-gate.sh<br/>瘦 shim → 委派全局权威 gate]
    end
    ...
    class I1,I2,I3,I4,I5,I6 install
    class S1,S2,S3,S4,S5 skill
    class H1,H2 hook
    class P1,P2,P3,P4 platform
    class R1,R2 runtime
    class B1,B2,B3,B4 tool
```
...
```mermaid
flowchart LR
    A[git commit] --> S{项目 .githooks<br/>是 shim?}
    S -- 是（v1.9.0+）--> AUTH[exec 全局权威 gate]
    S -- 否（老拷贝）--> STALE[运行本地冻结 gate]
    STALE --> B
    AUTH --> B{AGENT_MODE=1?}
    ...
    E -- 否 --> X2[阻断 commit ❌]
    E -- 是 --> F{Gate 2b: L1+ 机器<br/>REVIEW_LEVEL 标记?}
    F -- 否 --> X3[阻断: 异构审查必须标记 ❌]
    F -- 是 --> Z

    classDef block fill:#fef2f2,stroke:#b91c1c
    classDef pass fill:#f0fdf4,stroke:#15803d
    classDef shim fill:#f1f5f9,stroke:#475569
    class X1,X2,X3 block
    class Z pass
    class S,AUTH,STALE shim
```

**核心设计**：

- **只约束 agent**，不影响人类开发者——通过 `AGENT_MODE=1` 环境变量区分。
- **Gate 2b（v1.7.0+）**：在 L1+ 机器（有 opencode 或 codex 审查能力），review 文件必须携带 `<!-- REVIEW_LEVEL: Lx -->` 标记,阻止 agent 自审自批（"Opus reviews Opus"）。
- **shim 模型（v1.9.0+）**：项目级 `.githooks/agent-quality-gate.sh` 是 ~10 行瘦 shim,`exec` 全局权威 gate。升级 agent-gates 后**所有 shim 项目自动跟随**,不必逐仓 re-init。全局权威 gate 缺失时 shim exit 0 放行（humans pass through）——没装 agent-gates 的队友不受影响。
...
### 5.3 实施度盘点（v1.3.1 现状 — 已过时，见下方 v1.9.0 盘点）

> **以下表格是 v1.3.1 时的快照，保留作历史参考。v1.5.0-v1.9.0 大幅推进了 L1/L2/L3，当前实施度见 §5.3.1。**
...
### 5.3.1 实施度盘点（v1.9.0 现状）

v1.5.0-v1.9.0 连续 15 个版本把 L1/L2/L3 三层从"约 1/3"推到了"约 95%"。

| 层 | 设计要求 | agent-gates v1.9.0 实际 | 状态 |
|---|---|---|---|
...
| 运维 — opencode 可靠性 | opencode 空输出重试 + 孤儿 serve 清理 | `oc-review`（重试）+ `oc-reaper`（清理）+ doctor check_opencode_health（v1.8.0） | ✅ 100% |
| 运维 — 全局升级 | install --upgrade 一次升所有项目 | gate-shim 模型 + agent-gates-migrate + agent-gates-version（v1.9.0） | ✅ 100% |
...
| **v1.6.1** | 2026-06-02 | 并行提醒 hook | `memory-reminder.mjs` 新增 Parallelism Reminder：检测到 agent 写计划后把独立 todo 逐个串行时,注入提醒 |
| **v1.6.2** | 2026-06-02 | 并行触发校准 | 用真实会话 transcript 校准 detectPlanTimeTodos 门槛（0 completed + ≤1 in_progress + ≥1 pending） |
| **v1.6.3** | 2026-06-02 | 规则沉淀 | 沉淀 §9.2 "synthetic fixtures ≠ real-data evidence" 到 agent-workflow-rules |
| **v1.7.0** | 2026-06-02 | 异构审查强制 | Gate 2b：L1+ 机器上 review 必须携带 `REVIEW_LEVEL: Lx` 标记,阻止同模型自审自批 |
...
| 与 OpenSpec 协同 | 不挂钩 | CHECK 1/2 + Path A 路由 |
| 自检工具 | 无 | `doctor.sh` + `agent-gates-version` |
| 升级模型 | 手动替换 | 全局升级自动覆盖（v1.9.0 shim） |

**一句话总结**：
- **OpenSpec** = L1 规划层（说要做什么、为什么、怎么验收）
- **agent-superpowers** = L2 的纯 prompt 版本（轻量、靠 agent 自觉）
- **agent-gates** = L2（skill + memory hook）+ L3（CLI gate 硬阻断 + 异构审查强制）+ 运维（doctor/uninstall/多平台/审查路由/全局升级）的工程化落地。v1.9.0 与原整合方案的实施度已达约 95%（详见 §5.3.1）
...
同一条命令既能首装也能升级（幂等）。升级时 shim 项目自动跟随,无需逐仓 re-init。
...
2. 安装 pre-commit hook：将全局 gate-shim 复制到 `.githooks/agent-quality-gate.sh`（v1.9.0+ shim 模型,委派全局权威 gate）
...
├── init-deep-fallback/SKILL.md          # 层级 AGENTS.md 兜底（v1.5.2+）
...
<project>/.githooks/agent-quality-gate.sh  # 瘦 shim → exec 全局权威 gate（v1.9.0+）
                                           # 全局权威缺失时 exit 0 放行（humans pass through）
```
...
| Hook 触发了但 Memory 没存 | 没装 Memory skill | v1.5.4+ 已内置;跑 `install.sh --upgrade` |
| 升级了 agent-gates 但仓库行为没变 | 仓库用的是 v1.9.0 前的老拷贝 gate | `~/.agent-gates/bin/agent-gates-migrate --apply <项目根>`（一次性迁到 shim,之后自动跟随） |
| `agent-gates-version` 报 STALE | 老拷贝 gate 不跟随全局升级 | 同上,migrate 到 shim |
| Gate 2b 阻断但我已有 review | review 文件没标 `REVIEW_LEVEL: Lx` | 审查 agent 按 §9 模板输出时自动标;手动加 `<!-- REVIEW_LEVEL: L1 -->` |
| opencode 审查输出为空 | opencode P2 空输出 flake | 用 `~/.agent-gates/bin/oc-review`（自动重试） |
...
- 版本查看：`~/.agent-gates/bin/agent-gates-version`
- 批量迁移：`~/.agent-gates/bin/agent-gates-migrate <项目根>`

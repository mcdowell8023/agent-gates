...
## §18 并行优先（Parallelism-First）

**核心原则**：独立的工作不要一件一件串行做。但"并行"有**两种**机制，先分清用哪种——混淆这两者会让你被反复纠正。

### 18.1 两种并行机制（先分清，再动手）

| 机制 | 长什么样 | 用于 | 成本 |
| --- | --- | --- | --- |
| **A. 批量 tool call** | 同一条 assistant 消息里发多个 tool call（多个 Read / Edit / Bash） | 独立的轻量操作：读多个文件、改多个不相关文件、跑几条独立命令 | 几乎为零 |
| **B. 子 agent 并行** | 同一条消息里发多个 `Agent` / `Task` tool call | 独立的**重**工作流：每个都要读一堆文件、跑验证、写一片代码 | 高（起 agent + 独立 context） |

**判定线**：

```
有 ≥2 件独立的事要做？
  ├─ 每件都很轻（单文件读/改、一条命令）       → A. 同消息批量 tool call
  ├─ 每件都很重（要探索+实现+验证一整片）       → B. 并行子 agent
  └─ 有依赖链（A 的输出是 B 的输入）            → 才串行；并识别可分支点
```
...
- 一件一件单 tool call 做独立的读/改 → 应该 **A 批量**（别 Read→Edit→Read→Edit 串成一长串）
- 主 agent 亲自包揽 ≥ 3 件**重**任务 → 应该 **B 派子 agent**
- "我先做 A 再做 B 再做 C" → 检查 A B C 真有依赖吗？没有就并行
- 子 agent 后台跑期间主 agent 闲等

### 18.3 真实案例

- **案例 1（机制 B 该用没用）**：v1.5.1 收尾包含规则沉淀 / Vault 更新 / CHANGELOG / 测试验证 4 件互不依赖的重活，主 agent 包揽 3 件被用户纠正"分工反了"。正确：4 件并行派子 agent，主 agent 留最后 commit + push。
- **案例 2（A 和 B 没分清，被纠正三次）**：某 agent 做 5 个独立 UX 修复（不同 .vue/.spec 文件），一个一个串行改。用户说"为啥不并行"，agent 误以为是"批量 tool call"；用户说"不用子 agent"，agent 又反向理解成"别开子 agent"；第三次才懂用户问的是**子 agent 并行复核**。根因：把 §18.1 的 A 和 B 当成一回事。先分清机制，就不会来回误解。

### 18.4 与 §30-delegation 的关系

本节是**触发约束**（什么时候、用 A 还是 B）；30-delegation 是**操作规范**（派子 agent 的 prompt 格式要求）。两者互补。

### 18.5 `parallel-reminder` hook

当 agent-gates 安装后，`memory-reminder.mjs` 在你**创建 ≥3 个全 pending 的 todo**（即刚做完计划、尚未动工）时，注入标记 `[AGENT-GATES: Parallelism Reminder]` 的 system-reminder。**把它当强制检查点**：动手前按 §18.1 判定哪些独立、用 A 还是 B，别默认串行。提醒在计划阶段（全 pending 时）触发；一旦有 todo 转 in_progress 就不再触发。hook 无跨调用去重，实践中一次 TodoWrite 计划写入只触发一次。
